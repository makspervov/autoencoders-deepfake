    ...
    with torch.no_grad():
        for data, label in tqdm(dataloader, desc="Evaluating"):
            ...
            if is_openvino:
                ...
                # PyTorch inference
                data = data.to(device)
                # SAE outputs recon_batch and latent vector
...
def evaluate(
    ...
    plt.legend()
    plt.savefig(os.path.join(output_dir, "sae_error_distribution.png"))
    plt.close()
    ...
    plt.savefig(os.path.join(output_dir, "sae_roc_curve.png"))
    plt.close()
    ...
    if num_samples > 0:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "sae_reconstruction_grid.png"))
        plt.close()
