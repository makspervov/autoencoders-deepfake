...
class DeepFakeFaceDatasetSAE(Dataset):
    """
    Dataset class for loading face images directly from the Hugging Face datasets library.
    It expects the 'OpenRL/DeepFakeFace' dataset and is isolated for the SAE pipeline.
    """
...
def get_dataloaders_sae(
    ...
    Returns:
        train_loader: DataLoader with only real images (for SAE training).
    ...
    real_dataset = DeepFakeFaceDatasetSAE(
        is_fake=False, image_size=image_size, max_samples=5000
    )
    fake_dataset = DeepFakeFaceDatasetSAE(
